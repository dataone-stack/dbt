...
--     SUM(
--        (COALESCE(i.original_price, 0) - COALESCE(i.seller_discount, 0) - COALESCE(i.discount_from_voucher_seller, 0) -  sum(rd.so_tien_hoan_tra)) 
...
--     --   ELSE 0
--     -- END
--      sum(rd.so_tien_hoan_tra) * -1 AS so_tien_hoan_tra,
--     0 as nguoi_ban_hoan_xu,
--     CASE
--       WHEN sum(rd.so_tien_hoan_tra) > 0
--       THEN 0
--       ELSE sum(shopee_discount)
--     END
--     as tro_gia_shopee,
...
--     ops.tro_gia_shopee,
--     (ops.gia_goc + (ops.seller_tro_gia  + COALESCE(ops.so_tien_hoan_tra,0) -ops.tro_gia_shopee) + (ops.tro_gia_shopee + ((f.voucher_from_seller)*-1) + nguoi_ban_hoan_xu)) as doanh_thu_shopee,
--     (f.commission_fee *-1) + (f.service_fee *-1) + (f.seller_transaction_fee *-1) + (f.order_ams_commission_fee *-1) AS phu_phi

-- FROM {{ ref('t1_shopee_shop_wallet_total') }} vi 
-- LEFT JOIN {{ ref('t1_shopee_shop_fee_total') }} f
