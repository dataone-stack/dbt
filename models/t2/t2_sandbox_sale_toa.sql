WITH a AS (
  ...
  WHERE ngay_data_ve IS NOT NULL 
    AND nguon_doanh_thu != 'Sàn TMDT liên kết'
)
...
,b AS (
  SELECT 
    *,
    DATE(ngay_chot_don) AS ngay_tao_contact
  FROM {{ref("t2_sandbox_order_lines_toa")}}
  WHERE ngay_chot_don IS NOT NULL 
    AND order_confirm_name = 'Chốt đơn'
    AND nguon_doanh_thu != 'Sàn TMDT liên kết'
)
...
,c AS (
  ...
  SELECT 
    a.* REPLACE ( 0  AS so_sale, 0 as doanh_so_cu, 0 as doanh_so_moi, 0 as doanh_so),
    'Đơn Lead' as type_order_sale
  FROM a 
  ...
  SELECT 
    b.* REPLACE (0 AS so_lead),
    'Đơn Chốt' as type_order_sale
  FROM b
