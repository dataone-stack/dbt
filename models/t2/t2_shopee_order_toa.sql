WITH return_detail AS (
  ...
  UNNEST(item) AS i
  left join {{ref("t1_bang_gia_san_pham")}} as b on trim(i.variation_sku) = trim(b.ma_sku)
) 
,
...
,sale_detail AS (
  SELECT
    ...
    safe_divide(detail.discounted_price , ta.total_tong_tien_san_pham) * detail.final_shipping_fee AS final_shipping_fee,
...
--   company_lv1,
  ...
  phi_van_chuyen_thuc_te as phi_van_chuyen_thuc_te,
  phi_van_chuyen_tro_gia_tu_shopee as phi_van_chuyen_tro_gia_tu_san,
  phi_co_dinh as phi_co_dinh,
  phi_dich_vu as phi_dich_vu,
  phi_thanh_toan as phi_thanh_toan,
  phi_hoa_hong_tiep_thi_lien_ket as phi_hoa_hong_tiep_thi_lien_ket,
  tro_gia_tu_shopee as san_tro_gia,
  voucher_from_seller as voucher_from_seller,
...
COALESCE(tong_tien_san_pham, 0) - COALESCE(shopee_voucher, 0) - COALESCE(discount_from_coin, 0) - COALESCE(discount_from_voucher_seller, 0) - COALESCE(khuyen_mai_cho_the_tin_dung, 0) - phi_van_chuyen_nguoi_mua_tra - final_shipping_fee AS tien_khach_hang_thanh_toan,
return_status,
...
COALESCE(phi_co_dinh, 0) + COALESCE(phi_dich_vu, 0) + COALESCE(phi_thanh_toan, 0) + COALESCE(phi_hoa_hong_tiep_thi_lien_ket, 0) as phu_phi_shopee
