...
SELECT
    *,
    ROUND(thanh_tien - COALESCE(chiet_khau, 0) - COALESCE(giam_gia_san_pham, 0) + (COALESCE(gia_dich_vu_vc, 0) - COALESCE(phi_vc_ho_tro_khach, 0)),-1) AS tien_khach_hang_thanh_toan,
    ...
    ROUND((thanh_tien - COALESCE(chiet_khau, 0)),-1) -- + (COALESCE(gia_dich_vu_vc, 0) - COALESCE(phi_vc_ho_tro_khach, 0)))
     AS doanh_thu_ke_toan,

    CASE 
        WHEN loai_khach_hang = 'Khách hàng mới' 
        THEN ROUND(thanh_tien - chiet_khau,-1)
        ELSE 0
    END AS doanh_so_moi,
