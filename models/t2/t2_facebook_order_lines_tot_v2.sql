    ...
    COALESCE(
      SAFE_DIVIDE(
        ...
        *
        safe_cast(json_value(item, '$.quantity') as int64),
        NULLIF(tt.total_amount, 0)
      ) * ord.total_discount, 0) as giam_gia_don_hang_v2,
      COALESCE(
      SAFE_DIVIDE(
        safe_cast(json_value(item, '$.variation_info.retail_price') as int64)*
        safe_cast(json_value(item, '$.quantity') as int64),
    ...
    COALESCE(
      SAFE_DIVIDE(
        ...
        *
        safe_cast(json_value(item, '$.quantity') as int64),
        NULLIF(tt.total_amount, 0)
      ) * ord.shipping_fee, 0) as phi_van_chuyen_v2,
    ...
    COALESCE(
      SAFE_DIVIDE(
        ...
        *
        safe_cast(json_value(item, '$.quantity') as int64),
        NULLIF(tt.total_amount, 0)
      ) * ord.partner_fee, 0) as cuoc_vc_v2,
...
,order_delivered as (
    ...
    case
        ...
        then 0
        when brand = "An Cung"
        then ((gia_goc_sau_giam_gia_san_pham * so_luong) - khuyen_mai_dong_gia - giam_gia_don_hang_v2 + phi_van_chuyen_v2)
        -- when (gia_goc_sau_giam_gia_san_pham * so_luong) - giam_gia_don_hang + phi_van_chuyen < 50000
        -- then 0
        else ((gia_goc_sau_giam_gia_san_pham * so_luong) - giam_gia_don_hang_v2 + phi_van_chuyen_v2)
...
,order_returned as (
    ...
    case
        ...
        then 0
        when brand = "An Cung"
        then ((gia_goc_sau_giam_gia_san_pham * so_luong) - khuyen_mai_dong_gia - giam_gia_don_hang_v2 + phi_van_chuyen_v2) * -1
        -- when (gia_goc_sau_giam_gia_san_pham * so_luong) - giam_gia_don_hang + phi_van_chuyen < 50000
        -- then 0
        else ((gia_goc_sau_giam_gia_san_pham * so_luong) - giam_gia_don_hang_v2 + phi_van_chuyen_v2) * -1
    end as doanh_thu_ke_toan_v2,
