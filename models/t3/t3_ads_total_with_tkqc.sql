...
,
shop as (
  select date(ngay_tao_don) as ngay_tao_don,shop_id, sum(tien_khach_hang_thanh_toan) as doanh_thu_san from {{ref("t3_revenue_all_channel")}} --where shop_id in ('VNLCQUWAWR','1360134442') and date(ngay_tao_don) between'2025-11-09' and '2025-11-10'
  group by date(ngay_tao_don),shop_id
)

select a.*, c.doanh_thu_san from a
