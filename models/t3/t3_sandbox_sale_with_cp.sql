...
, a as(
  ...
  from t2_sale sale
  left join t3_ads ads on ads.ma_quan_ly = sale.ma_quan_ly and date(ads.date_start) = date(sale.ngay_tao_contact) and sale.type_order_sale = 'Đơn Lead'
  left join total_lead total on total.ma_quan_ly = sale.ma_quan_ly and date(total.ngay_tao_contact) = date(sale.ngay_tao_contact) and sale.type_order_sale = 'Đơn Lead'
