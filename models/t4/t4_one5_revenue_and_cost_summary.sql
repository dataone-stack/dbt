...
with
    ...
    revenue_toa AS (
        ...
        WHERE company = 'One5' -- status NOT IN  ('Đã hủy')
        GROUP BY DATE(ngay_tao_don), brand, channel, company --,ten_san_pham,sku_code
    ),
...
order by
    ...
    -- company,
    -- SUM(COALESCE(chiPhiAds, 0)) AS chi_phi_ads,
    -- SUM(COALESCE(doanhThuAds, 0)) + SUM(COALESCE(doanhThuLadi, 0)) AS
    -- doanh_thu_trinh_ads,
    -- SUM(COALESCE(doanhThuAds, 0)) AS doanhThuAds,
    -- SUM(COALESCE(doanhThuLadi, 0)) AS doanhThuLadi,
    ...
    -- )
    -- SELECT
    -- COALESCE(r.date_start, a.date_start, Cast(r_tot.date_start as date)) AS
    -- date_start,
    ...
    -- --     COALESCE(r.phi_van_chuyen_tro_gia_tu_san, 0) AS
    ...
    -- --     COALESCE(r.phi_hoa_hong_tiep_thi_lien_ket, 0) AS
    -- phi_hoa_hong_tiep_thi_lien_ket,
    -- --     COALESCE(r.phi_hoa_hong_quang_cao_cua_hang, 0) AS
    ...
    -- ON EXTRACT(YEAR FROM COALESCE(r.date_start, a.date_start)) = CAST(cir_max.year
    -- AS INT64)
    -- AND EXTRACT(MONTH FROM COALESCE(r.date_start, a.date_start)) =
    -- CAST(cir_max.month AS INT64)
    ...
    -- ON EXTRACT(YEAR FROM COALESCE(r.date_start, a.date_start)) =
    -- CAST(cir_max_ads.year AS INT64)
    -- AND EXTRACT(MONTH FROM COALESCE(r.date_start, a.date_start)) =
    -- CAST(cir_max_ads.month AS INT64)
